...
cornersX :: V2 Double -> Int -> V2 WorldPos -> [V2 WorldPos]
...
cornersY :: V2 Double -> Int -> V2 WorldPos -> [V2 WorldPos]
...
moveX f sz xdir pos =
  case any f $ cornersY sz xdir pos of
    ...
    True ->
      case xdir of
        -1 -> pos & _x .~ coerce ((tileToPos (posToTile pos) + coerce sz / 2 + epsilon) ^. _x)
...
moveY f sz ydir pos =
  case any f $ cornersX sz ydir pos of
    ...
    True ->
      case ydir of
        -1 -> pos & _y .~ coerce ((tileToPos (posToTile pos) + coerce sz / 2 + epsilon) ^. _y)
        0 -> pos -- already in the wall
